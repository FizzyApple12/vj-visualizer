...
@group(#{MATERIAL_BIND_GROUP}) @binding(6) var<storage, read> audiolink_data_lut: array<f32, 240>;
@group(#{MATERIAL_BIND_GROUP}) @binding(7) var<storage, read> audiolink_data_audio_data: array<vec4<f32>, 4096>;
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if coordinateGlobal.y > 3.0 && coordinateGlobal.y <= 5.0 {
        ...
        // Treble compensation
        mag *= (audiolink_data_lut[min(note, 239)] * AUDIOLINK_TREBLE_CORRECTION + 1);
